  ...
  <div>
    <div class="header">
      ...
      <div class="content">
        ...
        <div class="top">
          <span v-if="status" style="margin-left: 10px; color: grey">
            标签：
            <a
              href="javascript:;"
              style="color: rgb(0, 195, 255)"
              v-if="gedanInfo.tags.length === 0"
              >{{ gedanInfo.name }}</a
            >
            <a href="javascript:;" style="color: rgb(0, 195, 255)" v-else>{{
              gedanInfo.tags
            }}</a>
          </span>
...
export default {
  ...
  methods: {
    ...
    async getComment(item) {
      await this.$http.get("/comment/playlist?id=" + item.id).then((res) => {
